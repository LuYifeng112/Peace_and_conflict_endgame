...
NDiplomacy = {
	...
	MIN_TRUST_VALUE = -500,							-- Min trust value cap.
	...
	ASSUME_FACTION_LEADERSHIP_PP_COST = 600,				-- Political power cost to assume faction leadership
...
NCountry = {
	...
	SUPPLY_PATH_MAX_DISTANCE = 8,					-- When supply route reach more than X nodes, the manpower+equipment delivery speed reach 100% penalty.
	...
	REINFORCEMENT_DELIVERY_SPEED_MIN = 0.5,			-- The distance from the supply region to capital should affect the speed only a little bit. Main factor for penalty is overcrowded areas, and not the route length.
	REINFORCEMENT_EQUIPMENT_DELIVERY_SPEED = 0.012,	-- Modifier for army equipment reinforcement speed
	REINFORCEMENT_MANPOWER_DELIVERY_SPEED = 80,	-- Modifier for army manpower reinforcement delivery speed (travel time)
	...
	NUCLEAR_PRODUCTION_SCALE = 90,					-- +1 nuclear_production gives 1 nuke per year
	NUCLEAR_BOMB_DROP_WAR_SUPPORT_EFFECT_MAX_INFRA = 0.1, -- Reduce enemy national war support on nuking a province, the value scales with infrastructure up to this number
	...
	CONVOY_RANGE_FACTOR = 0.5,                        -- how much range affects convoy need
	LOCAL_MANPOWER_ACCESSIBLE_NON_CORE_FACTOR = 0.05,  -- accessible recruitable factor base
	...
	MIN_COUP_SUCCESS_STABILITY = 0.75,				-- Max stability when coup will happen
...
NTechnology = {
	MAX_SUBTECHS = 10,						-- Max number of sub technologies a technology can have.
	...
	DEFAULT_XP_RESEARCH_COST = 300,			-- default xp cost of a research to speed up the process
...
NPolitics = {
	...
	INSTANT_WIN_POPULARITY_WIN = 80, -- New party popularity
},
...
NMilitary = {
	...
	PLAN_MIN_AUTOMATED_EMPTY_POCKET_SIZE = 4,		-- The battle plan system will only automatically attack provinces in pockets that has no resistance and are no bigger than these many provinces
	PLAN_SPREAD_ATTACK_WEIGHT = 8.0,				-- The higher the value, the less it should crowd provinces with multiple attacks.
	...
	PLAN_PORVINCE_PORT_BASE_IMPORTANCE = 40.0,		-- Added importance for area defense province with a port
	PLAN_PORVINCE_PORT_LEVEL_FACTOR = 4.5,			-- Bonus factor for port level
	PLAN_PORVINCE_AIRFIELD_BASE_IMPORTANCE = 20.0,	-- Added importance for area defense province with air field
	PLAN_PORVINCE_AIRFIELD_POPULATED_FACTOR = 5.0,	-- Bonus factor when an airfield has planes on it
	...
	PLAN_AREA_DEFENSE_COASTAL_FORT_IMPORTANCE = 15.0,-- Used when calculating the calue of defense area provinces for the battle plan system
	...
	PLAN_EXECUTE_RUSH = 0,						-- When looking for an attach target, this score limit is required in the battle plan to consider province for attack
	...
	NUKE_DELAY_HOURS = 3,							-- How many hours does it take for the nuclear drop to happen
	PARADROP_PENALTY = -0.3, 						-- Combat penalty when recently paradropped
	PARADROP_HOURS = 6,							-- time paratroopers suffer penalties in combat
	COMBAT_SUPPLY_LACK_IMPACT = -0.5,				-- combat penalty if out of supply
	...
	FRONT_MIN_PATH_TO_REDEPLOY = 1,					-- If a units path is at least this long to reach its front location, it will strategically redeploy.
	...
	UNIT_UPKEEP_ATTRITION = 0.01,							--Constant attrition value applied to armies.
...
NNavy = {
	...
	RESOURCE_LENDLEASE_PRIORITY = 0.8,								-- Default convoy priority for export lend lease
	RESOURCE_EXPORT_PRIORITY = 4,									-- Default convoy priority for export trade
	RESOURCE_ORIGIN_PRIORITY = 0.5,									-- Default convoy priority for resources shipped internally
...
NAI = {
	...
	TRADEABLE_FACTORIES_FRACTION = 1.0,			-- Will at most trade away this fraction of factories.
	...
	WANTED_UNITS_INDUSTRY_FACTORY = 5.0, 		-- How many units a country wants is partially based on how much military industry that is available
	...
	PLAN_ATTACK_MIN_STRENGTH_FACTOR_MED = 0.75,	
	PLAN_ATTACK_MIN_ORG_FACTOR_HIGH = 0.6,		
	PLAN_ATTACK_MIN_STRENGTH_FACTOR_HIGH = 0.6,	
	PLAN_FRONTUNIT_DISTANCE_FACTOR = 10.0,		-- Factor for candidate units distance to front positions.
	PLAN_ATTACK_DEPTH_FACTOR = 1.0,				-- Factor applied to size or enemy being attacked.
	...
	SEND_VOLUNTEER_EVAL_MAJOER_POWER = 3.0, 	-- How willing major powers are to send volunteers.
	SEND_VOLUNTEER_EVAL_CONTAINMENT_FACTOR = 5.0, -- How much AI containment factors into its evaluation of sending volunteers.
	...
	POCKET_DISTANCE_MAX = 80000,				-- shortest square distance we bother about chasing pockets
	...
	REDEPLOY_DISTANCE_VS_ORDER_SIZE = 0.5,		-- Factor applied to the path length of a unit compared to length of an order to determine if it should use strategic redeployment
	UNIT_ASSIGNMENT_TERRAIN_IMPORTANCE = 30.0,	-- Terrain score for units are multiplied by this when the AI is deciding which front they should be assigned to
	...
	AIR_WING_REINFORCEMENT_LIMIT = 40,
	...
	START_TRAINING_EQUIPMENT_LEVEL = 0.2,               -- ai will not start to train if equipment drops below this level
	STOP_TRAINING_EQUIPMENT_LEVEL = 0.1,                -- ai will not train if equipment drops below this level
	...
	CONVOY_NEED_SAFETY_BUFFER = 1.50,					-- AI will try and keep 15% more convoys than what it needs.
	REGION_THREAT_PER_SUNK_CONVOY = 10,					-- Threat value per convoy sunk in a region. Decays over time.
	...
	AREA_DEFENSE_BASE_IMPORTANCE = 10,					-- Area defense order base importance value (used for determining order of troop selections)
	...
	MAX_SUPPLY_DIVISOR = 5.0,							-- To make sure the AI does not overdeploy divisions. Higher number means more supply per unit.
	MISSING_CONVOYS_BOOST_FACTOR = 18.0,					-- The more convoys a country is missing, the more resources it diverts to cover this.
	TRANSPORTS_PER_PARATROOPER = 3,					-- Air transports only duty is to drop paratroopers.
	...
	SCARY_LEVEL_AVERAGE_DEFENSE = -0.3,                 -- average front defense modifier to make it consider it as a PITA to go for
	ATTACK_HEAVILY_DEFENDED_LIMIT = 0.2,				-- AI will not launch attacks against heavily defended fronts unless they consider to have this level of advantage (1.0 = 100%)
	HOUR_BAD_COMBAT_REEVALUATE = 12,                   -- if we are in combat for this amount and it goes shitty then try skipping it 
	...
	FRONT_UNITS_CAP_FACTOR = 18.0,						-- A factor applied to total front size and supply use. Primarily effects small fronts
	MAX_DIST_PORT_RUSH = 500.0,							-- If a unit is in enemy territory with no supply it will consider nearby ports within this distance.
